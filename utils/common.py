...
# 🔧 修复 Python 3.10 兼容性：MutableMapping 已迁移到 collections.abc
...
def flatten_config(config: dict, parent_key: str = "", separator: str = "/") -> Dict[str, Any]:
    """
    Flatten nested config dicts into a flat dict for logging.
    Supports nested dictionaries (MutableMapping).
    """
    items = []
    for k, v in config.items():
        new_key = parent_key + separator + k if parent_key else k
