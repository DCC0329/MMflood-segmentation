...
#MorphologyTransform：做 mask 的形态学处理
# 去掉小噪声
# 填掉小洞
...
def _rgb_ratio(data: np.ndarray):
    """False-color RGB formula taken directly from Sentinel-hub. 生成一个 假的 RGB 合成
    """
    vv, vh = data[0], data[1]
    r = vv / 0.28
    g = vh / 0.06
...
def _process_tiff(image_id: str,
    ...
    “读一张大 tiff → 可选预处理 → 按 tiler 切成很多小 tile → 写到 dem/sar/mask 子目录里”
